...
function Norme()
	...
	let s:errornorm += matchadd('MyError', '\(^\t\(\i\+\|\i\+ \)\+\)\@<= \ze\**\i\+;', -1)

	" TAB BEFORE ','
	let s:errornorm += matchadd('MyError', '\s\ze,', -1)
	" TAB BEFORE '_'
	let s:errornorm += matchadd('MyWarning', ' \ze)', -1)

	" TAB AFTER '('
	let s:errornorm += matchadd('MyWarning', '(\zs ', -1)
	...
	" no MAJ after define
	let s:errornorm += matchadd('MyError', '\<define\> \zs\(\<\([0-9A-Z_]\)*\>\)\@!\i*', -1)
