	...
	if filereadable(l:file)
		...
		while l:line <= line('$')
			if !empty(matchstr(getline(l:line), '^\i.*')) && empty(matchstr(getline(l:line), '^static')) && empty(matchstr(getline(l:line), '^typedef')) && empty(matchstr(getline(l:line), '.*;$'))
				if empty(l:listprot)
					call insert(l:listprot, getline(l:line))
				else
					call add(l:listprot, getline(l:line))
				endif
			endif
			let l:line += 1
		endwhile
		...
		while !empty(matchstr(getline(line('.')), '^\i.*;$')) && line('.') < line('$') && empty(matchstr(getline(l:line), '^typedef')) && !empty(matchstr(getline(l:line), ');$'))
			exe 'normal dd'
		endwhile
		for prot in l:listprot
			exe "normal A". prot . ";\n"
